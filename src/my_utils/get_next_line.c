...
int make_line_bigger(char **line, char *buffer, int size)
{
    char *tmp = *line;
    int i = 0;
    ...
    if (*line == NULL) {
        *line = malloc(sizeof(char) * (size + 1));
        ...
        while (tmp[i++] != '\0');
        *line = malloc(sizeof(char) * (i + size));
        if (*line == NULL)
            return (1);
...
int put_rest_buffer_in_line(char *buffer, char **line, char *ptr)
{
    int i = 0;
...
char *gnl_get_return_value(char *line, int nb_read)
{
...
char *get_next_line(int fd)
{
    ...
    while (nb_read > 0 && ptr == NULL) {
        ...
        if (ptr == NULL && nb_read != -1
            && make_line_bigger(&line, buffer, nb_read) == 1)
            return (NULL);
    }
    if (ptr != NULL)
